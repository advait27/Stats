# stl_Kokru_Dharmadhikari_Sinha_compare.R
# MIS41120 Statistical Learning — Practical Assignment
# Stage 7a: pull all the model results together and compare them.
#
# The earlier stages each saved a one-row-per-model metrics file. Here we
# stack them into a single leaderboard and produce the tables/figures the
# report refers to: leaderboard sorted by AUC, bar charts of AUC and of
# F1/recall, an interpretability scatter (features used vs AUC), a small
# timing table, and a printed narrative answering the comparison questions.
#
# All metrics files share the same 9 columns:
#   method, accuracy, auc, recall, specificity, precision, f1,
#   balanced_accuracy, n_features_used
# Self-contained, relative paths, seeded. Run from the project root.
